            ...
            if teacher.get("credentials") == "Active":
                ...
                for grade in teacher.get("grades") or []:
                    for subject in teacher.get("subjects") or []:
                        matching_principal_ids = (
                            open_role_lookup.get(geography, {})
                            .get(grade, {})
                            .get(subject)
                            or []
                        )
                        for principal_id in set(matching_principal_ids).difference(
                            already_matched_principal_ids
                        ):
                            principal_queue = principal_queues_by_id.setdefault(
                                principal_id, empty_principal_queue(principal_id)
                            )
...
def empty_principal_queue(principal_id):
    return {"principalId": principal_id, "principalQueue": []}
