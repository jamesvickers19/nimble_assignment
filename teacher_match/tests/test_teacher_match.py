...
def to_order_independent(principal_queues):
    if "queues" in principal_queues:
        ...
        principal_queues["queues"] = sorted(
            principal_queues["queues"], key=lambda d: d["principalId"]
        )
...
class TeacherMatchTest(unittest.TestCase):
    ...
    def exercise_match_teachers_to_principals(
        self, teachers, principals, expected_principal_queues, msg=None
    ):
        expected = {"queues": expected_principal_queues}
        to_order_independent(expected)
        actual_principal_queues = match_teachers_to_principals(
            {"teachers": teachers, "principals": principals}
        )
    ...
    def test_no_teachers_or_principals(self):
        self.exercise_match_teachers_to_principals(
            teachers=[], principals=[], expected_principal_queues=[]
        )
    ...
    def test_no_principals(self):
        self.exercise_match_teachers_to_principals(
            teachers=[should_match_teacher(123)],
            principals=[],
            expected_principal_queues=[],
        )
    ...
    def test_geography_mismatch(self):
        ...
        principal = should_match_principal(principal_id)
        self.exercise_match_teachers_to_principals(
            teachers=[teacher], principals=[principal], expected_principal_queues=[]
        )
    ...
    def test_single_match(self):
        ...
        self.exercise_match_teachers_to_principals(
            ...
            principals=[principal],
            expected_principal_queues=[
                {"principalId": principal_id, "principalQueue": [teacher_id]}
            ],
        )
    ...
    def test_teachers_matched_to_multiple_principals(self):
        ...
        principal_1 = {
            ...
            "location": geography,
            "openRoles": [
                {"subject": "Science", "grade": "2nd grades"},  # matches teacher_1
        ...
        principal_2 = {
            ...
            "location": geography,
            "openRoles": [
                {"subject": "English", "grade": "2nd grades"},
                {
        ...
        expected = [
            {
                ...
                "principalQueue": [teacher_id_2, teacher_id_1],
            },
            {
                "principalId": principal_id_1,
                "principalQueue": [teacher_id_1, teacher_id_2],
            },
        ]
        self.exercise_match_teachers_to_principals(
            teachers=[teacher_2, teacher_1, not_matched_teacher],
        ...
        for possible_dir in os.listdir(os.path.join(test_dir, base_dir)):
            ...
            if os.path.isdir(example_dir):
                ...
                if os.path.exists(input_file) and os.path.exists(expected_output_file):
                    ...
                    self.assertTrue(
                        os.path.exists(created_output_file),
